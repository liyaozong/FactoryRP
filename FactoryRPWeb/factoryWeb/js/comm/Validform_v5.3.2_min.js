...
(function(d, f, b) {
	...
	a.util = {
		...
		check: function(v, t, n) {
			...
			if (!s.passed) {
                r.css('border','1px solid #FF6600')
                if(r.parent().find('span').hasClass('error_msg_tips')){
                    var navigatorName = "Microsoft Internet Explorer";
                    if(navigator.appName == navigatorName){
                        r.siblings(".error_msg_tips").remove();
                    }else{
                        r.siblings(".error_msg_tips").remove();
				...
				if (!n) {
					a.util.showmsg.call(v, s.info, o.tiptype, {
						...
						"bycheck");  ! o.tipSweep && r.parent().append("");
				}
				return false
			}
					...
					success: function(x) {
						if (d.trim(x.status) === "y") {
							...
							a.util.showmsg.call(v, m.attr("sucmsg") || v.data("tipmsg").r || e.r, o.tiptype, {
								...
								"bycheck");
                            /*r.css('border','1px solid #ccc');
							r.parent().find('span').removeClass("Validform_error");*/
							g = null;
			...
			if (!n) {
				a.util.showmsg.call(v, s.info, o.tiptype, {
					...
					"bycheck");
//                create BY jayvenLee--2016.02.01,验证成功颜色修改为灰色，默认为绿色
                r.css('border','1px solid #ccc')
		...
		submitForm: function(o, l, k, r, t) {
			...
			w.find("[datatype]").each(function() {
				...
				if (!n.passed) {
                    d(this).css("border","1px solid #FF6600")
				...
				//A.parent().find('span').removeClass("Validform_error");
				g = null
			});
			if (o.showAllError) {

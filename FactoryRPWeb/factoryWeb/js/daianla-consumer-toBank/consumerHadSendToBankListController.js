 ...
 */
consumerToBankApp.controller('hadSendToBankOrderListController',function ($scope,$http,$filter,$cookies, UrlService,$resource, $location, $state, hadSendToBankListService, ToBankOrderDetailService,ToBankLoanOrderDetailService) {
...
//        alert($(this).val());
        $cookies.put('currentPageHadSend',$(this).val());
    });
    $(".pagination").click(function(){
//        alert($(".page-total input").val());
        $cookies.put('currentPageHadSend',$(".page-total input").val());
    });
    $(".page-total select").change(function(){
        var items =$(".page-total select").val();
        var ss = items.substring(7,items.length);
        $cookies.put('itemsPerPageHadSend',ss);
    });
    var currentPage;
    var itemsPerPage;
    if($cookies.get('currentPageHadSend')==null||$cookies.get('currentPageHadSend')==''||$cookies.get('currentPageHadSend')==undefined){
        currentPage=1;
    }else{
        currentPage=$cookies.get('currentPageHadSend');
    }
    if($cookies.get('itemsPerPageHadSend')==null||$cookies.get('itemsPerPageHadSend')==''||$cookies.get('itemsPerPageHadSend')==undefined){
        itemsPerPage=15;
    }else{
        itemsPerPage=$cookies.get('itemsPerPageHadSend');
    }
    $scope.paginationConf = {
        currentPage: currentPage,
        itemsPerPage: itemsPerPage
    };
    $scope.orderList = hadSendToBankListService.getOrderList();

    /* 从总配置中查询优啦产品信息和渠道产品信息 start queryListProduct*/
    $scope.queryListProducts=function(){
        hadSendToBankListService.queryListProduct({}, function(response){
    ...
    $scope.queryListOrder=function(){
        hadSendToBankListService.queryListOrderStatus({}, function(response){
            if(response.status==0&&response.obj!=null&&response.obj!=''&&response.obj!=undefined){
    ...
    $("#cusName").blur(function(){
        $cookies.put('cusNameHadSend',$(this).val());
    });
    $scope.cusName=$cookies.get('cusNameHadSend');
    //用户电话
    $("#telephone").blur(function(){
        $cookies.put('telephoneHadSend',$(this).val());
    });
    $scope.telephone=$cookies.get('telephoneHadSend');

    //用户电话批量
    $("#userPhones").blur(function(){
        if($(this).val()!=''){
            $cookies.put('userPhonesHadSend',$(this).val().match(/\d+/g).join(','));
        }else{
            $cookies.put('userPhonesHadSend',$(this).val());
        }
    });
    $scope.userPhones=$cookies.get('userPhonesHadSend');

    //创建放款单开始时间
    $("#startDate").blur(function(){
        $cookies.put('startDateHadSend',$(this).val());
    });
    $scope.startDate=$cookies.get('startDateHadSend');
    $("#startDate").val($scope.startDate);

    //创建放款单结束时间
    $("#endDate").blur(function(){
        $cookies.put('endDateHadSend',$(this).val());
    });
    $scope.endDate=$cookies.get('endDateHadSend');
    $("#endDate").val($scope.endDate);

    //产品类型
    $("#productId").blur(function(){
        $cookies.put('productIdHadSend',$(this).val());
    });
    $scope.productId=$cookies.get('productIdHadSend');

    //资金通道
    $("#channelProductCode").blur(function(){
        $cookies.put('channelProductCodeHadSend',$(this).val());
    });
    $scope.channelProductCode=$cookies.get('channelProductCodeHadSend');

    //中征码
    $("#zzCodeExists").blur(function(){
        $cookies.put('zzCodeExistsHadSend',$(this).val());
    });

    $scope.zzCodeExists=$cookies.get('zzCodeExistsHadSend');
    //备注状态
    $scope.putFlagToCook=function(res){
        $cookies.put('remarkFlagHadSend',res);
    };
    $scope.remarkFlag=$cookies.get('remarkFlagHadSend');
...
//        $scope.paginationConf.itemsPerPage=15;
        ...
        $cookies.remove('cusNameHadSend');
        $cookies.remove('telephoneHadSend');
        $cookies.remove('userPhonesHadSend');
        $cookies.remove('startDateHadSend');
        $cookies.remove('endDateHadSend');
        $cookies.remove('productIdHadSend');
        $cookies.remove('channelProductCodeHadSend');
        $cookies.remove('remarkFlagHadSend');
        $cookies.remove('zzCodeExistsHadSend');
    ...
    $scope.onQuery=function () {
        ...
        hadSendToBankListService.queryOrder({
...
//        var par={params:pa};
    ...
    $scope.onBzContent = function () {
        hadSendToBankListService.queryFlagBz({

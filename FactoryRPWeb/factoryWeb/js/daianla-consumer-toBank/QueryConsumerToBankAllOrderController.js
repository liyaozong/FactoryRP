 ...
 */
consumerToBankApp.controller('ToBankAllOrderController',function ($scope,UrlService,$http,$filter,$cookies,$resource, $location, $state, ToBankAllOrderService, ToBankOrderDetailService,ToBankLoanOrderDetailService) {
...
//        alert($(".page-total input").val());
    ...
    $scope.orderList = ToBankAllOrderService.getOrderList();

    /* 从总配置中查询优啦产品信息和渠道产品信息 start queryListProduct*/
    $scope.queryListProducts=function(){
        ToBankAllOrderService.queryListProduct({}, function(response){
    ...
    $scope.queryListOrder=function(){
        ToBankAllOrderService.queryListOrderStatus({}, function(response){
            if(response.status==0&&response.obj!=null&&response.obj!=''&&response.obj!=undefined){
...
//        $scope.paginationConf.itemsPerPage=15;
    ...
    $scope.onQuery=function () {
        ...
        ToBankAllOrderService.queryOrder({
...
//        var par={params:pa};
    ...
    $scope.onBzContent = function () {
        ToBankAllOrderService.queryFlagBz({

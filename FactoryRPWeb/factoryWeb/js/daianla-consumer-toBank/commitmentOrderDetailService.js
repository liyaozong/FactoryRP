consumerToBankApp.factory('commitmentOrderDetailService', function($resource, $log, UrlService) {
    var allOrderList=[];
    var curHandleOrder = {};
...
//      var creditResults = $resource("http://192.168.1.40:8080/" + 'credit/result');
    ...
    /*放款环节-审批提交 end*/
    /* 查询承诺函信息 start*/
    var _queryCommitmentInfo = function(params, successFunc){
        var CommitmentInfos = $resource(UrlService.getUrl('commitmentService') + params);
        CommitmentInfos.get(function (data) {
            if (successFunc) {
                successFunc(data);
            }
        }, function (err) {
    ...
    /*保存已上传的承诺函 submit start*/
    var _commitmentSubmit = function(params, successFunc){
        var commitmentSubmits = $resource(UrlService.getUrl('commitmentService') + 'submit');
        commitmentSubmits.save(params,function (data) {
            if (successFunc) {
                successFunc(data);
            }
        }, function (err) {
    ...
    /*保存已上传的承诺函 end*/
    /*审批承诺函 start*/
    var _commitmentSubmitAudit = function(code,params, successFunc){
        var commitmentSubmitAudits = $resource(UrlService.getUrl('commitmentService') + 'audit/'+code);
        commitmentSubmitAudits.save(params,function (data) {
            if (successFunc) {
                successFunc(data);
            }
        }, function (err) {
    ...
    return {
        ...
        queryCommitmentInfo:_queryCommitmentInfo,
        commitmentSubmit:_commitmentSubmit,
        commitmentSubmitAudit:_commitmentSubmitAudit,

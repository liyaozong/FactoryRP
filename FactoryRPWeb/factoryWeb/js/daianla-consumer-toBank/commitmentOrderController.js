 ...
 */
consumerToBankApp.controller('commitmentOrderController',function ($scope,UrlService,$http,$filter,$cookies,$resource, $location, $state, commitmentOrderService, commitmentOrderDetailService) {
    $scope.WebURL=UrlService.getUrl('manageAudit');
    var data =localStorage.getItem('dataWait');
    $scope.dataWait=data;
    $scope.commitmentStatusList=[{'code':'NOT_SUBMITTED','name':'补全资料'},{'code':'AUDITING','name':'审核中'},{'code':'PASSED','name':'审核通过'}];
...
//        alert($(".page-total input").val());
    ...
    $scope.orderList = commitmentOrderService.getOrderList();

    /* 从总配置中查询优啦产品信息和渠道产品信息 start queryListProduct*/
    $scope.queryListProducts=function(){
        commitmentOrderService.queryListProduct({}, function(response){
    ...
    $scope.queryListOrder=function(){
        commitmentOrderService.queryListOrderStatus({}, function(response){
            if(response.status==0&&response.obj!=null&&response.obj!=''&&response.obj!=undefined){
    ...
    /* 查询承诺函信息 start*/
    $scope.showCommitmentOrderStatus=function(res){
        commitmentOrderService.queryCommitmentInfo(res, function(response){
            if(response.status==0&&response.obj!=null&&response.obj!=''&&response.obj!=undefined){
                $scope.commitmentOrderInfo = response.obj;
                console.log($scope.commitmentOrderInfo);
                $scope.orderList.forEach(function(items){
                    if(items.code==res){
                        items.commitmentOrderStatus=$scope.commitmentOrderInfo.status;
                    }
                })
            }else{
...
//        $scope.paginationConf.itemsPerPage=15;
    ...
    $scope.onQuery=function () {
        ...
        commitmentOrderService.queryOrder({
...
//        var par={params:pa};
    ...
    $scope.onBzContent = function () {
        commitmentOrderService.queryFlagBz({
...
//
//        }
        $state.go('main.CommitmentOrderOrderDetail',{'hideData':0});
        commitmentOrderDetailService.setCurHandleOrder(order);
    }
});
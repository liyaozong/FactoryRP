 ...
 */
factoryParameterSettingApp.controller('otherOptionsSettingController',function ($scope,UrlService,$http,$filter,$cookies,$resource, $location, $state, factoryParameterSettingService) {
    $scope.WebURL=UrlService.getUrl('factoryServe');
//    if($location.path()=='/main/modelToolsManage'){
//        $("#menuLeft .leftmenu .modelToolsManage .menuson").css('display','block');
//        $("#menuLeft .leftmenu .modelToolsManage").removeClass('hide');
//        $("#menuLeft .leftmenu .modelToolsManage").siblings().addClass('hide');
//    }else{
...
//    }
    ...
    /*查询所有 设备相关字典名称code start*/
    $scope.viewDeviceDicCodes=function(){
        factoryParameterSettingService.viewDeviceDicCode(function(response){
            if(response.errorCode=='000000'){
                if(response.data!=null&&response.data!=''&&response.data!=undefined){
                    $scope.deviceDicCodes=response.data
                }else{
                    $scope.deviceDicCodes=[];
                }
            }else{
    ...
    $scope.viewDeviceDicCodes();
    /*查询所有 设备相关字典名称code end*/
    /*根据code查询字典参数列表 queryDeviceDictionary start*/
    $scope.deviceDicCode='device_measuring_unit';
    $scope.showChangeName=function (code) {
        factoryParameterSettingService.queryDeviceDictionary({
            ...
            if(response.errorCode=='000000'){
                if(response.data!=null&&response.data!=''&&response.data!=undefined){
                    ...
                    $scope.deviceDicName=$scope.temp.join('\n');
                }else{
                    $scope.deviceDicName=[];
                }
            }else{
        ...
        $scope.titleName= $("#deviceDicCode").find("option:selected").text();
    };
